...
{
        int index,M,N;
        Position Pa,Pb,Pp,Temp,result;
        M=CountSize(A);
        N=CountSize(B);
        M=M<=N?M:N;
	HashTable Table;
	Table=InitializeTable(M*N);
	result=Pp=Initiate(Pp);
        Pa=A->Next;
        while(Pa!=NULL)
        {
                Pb=B->Next;
                while(Pb!=NULL)
                {
                        Insert_table(Pa->Coefficient*Pb->Coefficient,Pa->Exponent+Pb->Exponent,Table);
                        Pb=Advance(Pb);
                }
                Pa=Advance(Pa);
        }
	...
	{
		Temp=Table->TheLists[index];
		while(Temp!=NULL)
		{
			Insert(Temp->Coefficient,Temp->Exponent,result,Pp);
			Pp=Advance(Pp);
			Temp=Temp->Next;
		}
	}
