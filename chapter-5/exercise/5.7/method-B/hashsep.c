...
{
	List L=H->TheLists[Hash(Exponent,H->TableSize)];
	return FindList(Exponent,L);
}
void Insert_table(ElementType Coefficient,ElementType Exponent,HashTable H)
{
	int index=Hash(Exponent,H->TableSize);
	List L;
	Position P;
	P=Find(Coefficient,Exponent,H);
	if(P)
	{
		P->Coefficient=P->Coefficient+Coefficient;
	}
	else
	{
